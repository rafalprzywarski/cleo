...
TEST_F(eval_test, syntax_quote_should_resolve_symbols_in_lists)
{
    in_ns(create_symbol("cleo.eval.syntax-quote.test"));
    Root val, ex;
    ex = read_str("()");
...
TEST_F(eval_test, syntax_quote_should_resolve_symbols_in_maps)
{
    in_ns(create_symbol("cleo.eval.syntax-quote.test"));
    Root val, ex;
    ex = read_str("{}");
    val = read_str("`{}");
    val = eval(*val);
    EXPECT_EQ_VALS(*type::PersistentHashMap, get_value_type(*val));
    EXPECT_EQ_VALS(*ex, *val);
...
TEST_F(eval_test, unquote_splicing_should_splice_a_sequence_into_a_list)
{
    in_ns(create_symbol("cleo.eval.syntax-quote.unquote-splicing.test"));
    define(create_symbol("cleo.eval.syntax-quote.unquote-splicing.test", "abc"), create_keyword("abc"));
...
TEST_F(eval_test, unquote_splicing_should_splice_a_sequence_into_a_set)
{
    in_ns(create_symbol("cleo.eval.syntax-quote.unquote-splicing.test"));
    define(create_symbol("cleo.eval.syntax-quote.unquote-splicing.test", "abc"), create_keyword("abc"));
